...
bool statusbars::draw()
{
	...
	this->draw_bar(2, "Food", u.uhunger/(float)1000, 150/float(1000),  CLR32_ORANGE);
	this->draw_bar(3, "Weight", (inv_weight()+weight_cap())/(float)weight_cap(), 0, CLR32_BROWN);
...
void statusbars::draw_bar(int i, std::string label, float lvlMain, float lvlSecondary, Uint32 color)
{
	...
	vulture_rect(x, y+barh*i, x+w, y+barh*i+barh, CLR32_GRAY70);
}
